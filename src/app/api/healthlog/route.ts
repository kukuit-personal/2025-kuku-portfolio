...
export async function GET(req: Request) {
  try {
    ...
    const skip = Number.isFinite(Number(searchParams.get("skip")))
      ? Number(searchParams.get("skip"))
      : 0;
    const take = Number.isFinite(Number(searchParams.get("take")))
      ? Math.min(Number(searchParams.get("take")), 200)
      : 100;
    ...
    const where: any = {
      ...
      ...(from || to
        ? {
            // lọc theo khoảng ngày (string so sánh lexicographical vì YYYY-MM-DD)
            date: {
              ...(from ? { gte: from } : {}),
              ...(to ? { lte: to } : {}),
            },
...
export async function POST(req: Request) {
  try {
    ...
    const data = {
      ...
      // số: cho phép number hoặc string số
      ...
      status:
        body.status === "disabled" ? Status.disabled : Status.active,
    };
